...
class CitiesTempData:
    ...
    def get_datetimes(
        self, selection_only: bool = True, as_bool: bool = False
    ) -> pd.Series:
    ...
    def get_data(
        ...
        if year_start is not None:
            out_df = out_df.loc[
                [True if year_start <= dt.year else False for dt in out_df.index]
            ]

        if year_end is not None:
            out_df = out_df.loc[
                [True if dt.year <= year_end else False for dt in out_df.index]
            ]

        if month is not None:
            out_df = out_df.loc[
                [True if dt.month == month else False for dt in out_df.index]
            ]

        if date_start is not None:
            out_df = out_df.loc[
                [True if dt >= date_start else False for dt in out_df.index]
            ]

        if date_end is not None:
            out_df = out_df.loc[
                [True if dt <= date_end else False for dt in out_df.index]
            ]
