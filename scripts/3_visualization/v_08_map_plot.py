...
date_show = st.sidebar.select_slider("Show date", options=options, value=options[-1])
...
st.pydeck_chart(
    pdk.Deck(
        ...
        initial_view_state=pdk.ViewState(latitude=-17, longitude=-65, zoom=4, pitch=50),
        # Defining the hovertext from the DataFrame columns.
        tooltip={
            "text": "{city} {date}: {temperature}\nAll-time low: {all_time_low}\nAll-time high: {all_time_high}"
        },
