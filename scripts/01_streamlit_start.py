...
selected_files_list = st.sidebar.multiselect(
    "Select file names to view", options=file_name_list, default=file_name_list
)
...
if show_mean_bool:
    ...
    for file_name in file_dict:
        if year in file_dict[file_name].index:
            # Build mean df.
            mean_df = pd.concat(
                [mean_df, pd.DataFrame({file_name: file_dict[file_name].loc[year]})],
                axis=1,
            )
...
for file_name in (fname for fname in file_dict if fname in selected_files_list):
    if year in file_dict[file_name].index:
        ...
        fig.add_trace(
            go.Scattergl(
                x=file_dict[file_name].columns,
                y=file_dict[file_name].loc[year],
                name=file_name,
            )
        )
